...
import { requestApiText, getAdminArena1Teams, createAdminArena1Team, deleteAdminArena1Team, adminGradeArena1Submission, getAdminArena1Questions, adminStartArena1, adminEndArena1, adminPostArena1DiscordReport, Arena1Question, Arena1SlotResult } from '@/lib/api';
...
export function AdminArena1Panel({ token }: { token: string }) {
  ...
  const submissionPath = gradingSlot
    ? `/admin/a1/submissions/${encodeURIComponent(gradingSlot.teamId)}/${gradingSlot.slot}`
    : '';
  ...
  useEffect(() => {
    if (gradingSlot) {
      setCodeText('Loading...');
      setSubmissionHtml('');
      requestApiText(submissionPath, token)
        .then(text => {
          setCodeText(text);
          setSubmissionHtml(text);
        })
        .catch(() => {
          setCodeText('Failed to load code');
          setSubmissionHtml('<!DOCTYPE html><html><body style="font-family: sans-serif; padding: 24px;">Failed to load preview</body></html>');
        });
    }
  }, [gradingSlot, submissionPath, token]);
